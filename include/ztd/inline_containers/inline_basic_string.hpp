...
#ifndef ZTD_INLINE_CONTAINERS_INLINE_BASIC_STRING_HPP
#define ZTD_INLINE_CONTAINERS_INLINE_BASIC_STRING_HPP
...
namespace ztd {
	...
	: private ::ztd::inline_vector<_Ty, _MaxLength + 1>,
	  public ::ztd::__fc_detail::__sv_conversion<_Ty, ::ztd::inline_basic_string<_Ty, _MaxLength>> {
	private:
		using __base = ::ztd::inline_vector<_Ty, _MaxLength + 1>;
	...
	public:
		...
		constexpr inline_basic_string() noexcept : __base() {
			this->_M_set_null_terminator();
		}

		constexpr inline_basic_string(::std::initializer_list<value_type> __values) noexcept : __base(__values) {
			this->_M_set_null_terminator();
		}

		constexpr inline_basic_string(size_type __count) noexcept : __base(__count) {
			this->_M_set_null_terminator();
		}

		constexpr inline_basic_string(size_type __count, const value_type& __value) noexcept
		...
		template <typename _First, typename _Last,
		     ::std::enable_if_t<!::std::is_integral_v<_First>                    // cf
		          ...
		          >* = nullptr>
		constexpr inline_basic_string(_First __first, _Last __last) noexcept
	...
	class inline_basic_string<_Ty, 0> {
	public:
		...
		constexpr inline_basic_string() noexcept {
		}

		constexpr inline_basic_string(::std::initializer_list<value_type> __values) noexcept {
			ZTD_ASSERT_MESSAGE("there must be no values", __values.size() < size_type(1));
		}

		constexpr inline_basic_string(size_type __count) noexcept {
			ZTD_ASSERT_MESSAGE("there must be no values", __count < size_type(1));
		}

		template <typename... _Args>
		reference emplace_back(_Args&&...) noexcept {
			ZTD_ASSERT_MESSAGE("there is no capacity in this inline_basic_string", false);
		...
		iterator emplace(const_iterator, _Args&&...) noexcept {
			ZTD_ASSERT_MESSAGE("nothing can be emplaced in this inline_basic_string", false);
			return iterator(nullptr);
		}

		constexpr iterator insert(const_iterator, value_type&&) noexcept {
			ZTD_ASSERT_MESSAGE("nothing can be inserted in this inline_basic_string", false);
			return iterator(nullptr);
		}
		...
		constexpr iterator erase(const_iterator) noexcept {
			ZTD_ASSERT_MESSAGE("there is nothing to erase in this inline_basic_string", false);
			return iterator(nullptr);
		}
		...
		constexpr void resize(size_type __count) noexcept {
			ZTD_ASSERT_MESSAGE("there is no capacity in this inline_basic_string to resize to anything but 0",
			     __count < size_type(1));
		}
	...
	constexpr bool operator==(const ::ztd::inline_basic_string<_LeftTy, _LeftMaxLength>& __left,
	     const ::ztd::inline_basic_string<_RightTy, _RightMaxLength>& __right) noexcept {
