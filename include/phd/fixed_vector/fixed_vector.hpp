...
#ifndef PHD_FIXED_VECTOR_FIXED_VECTOR_HPP
#define PHD_FIXED_VECTOR_FIXED_VECTOR_HPP
...
namespace phd { inline namespace __0 {
	...
	public:
		...
		using iterator        = __detail::__wrap_pointer<__detail::__uninit<_Ty>>;
		using const_iterator  = __detail::__wrap_pointer<const __detail::__uninit<_Ty>>;
	...
	public:
		...
		iterator emplace(const_iterator __where, _Args&&... __args) noexcept(
			...
			__alloc_traits::construct(__al, to_address(__where_last), ::std::move(*__from_where_p));
			...
			__alloc_traits::construct(__al, to_address(__where_first), ::std::forward<_Args>(__args)...);
		}
	...
	private:
		size_type _M_size;
		__detail::__uninit<value_type> _M_elements[_Capacity];
...
#endif // PHD_FIXED_VECTOR_FIXED_VECTOR_HPP
